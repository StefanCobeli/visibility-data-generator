...
export default class ParticleHelper extends EventEmitter {
  constructor() {
    ...
    //     color: '#6F31EC',
    // })//before kAZI 09/04/2025 particle color purple
    this.material = new THREE.MeshBasicMaterial({
      size: 50,
      color: "#d95f02",
    });
  ...
  plotParticlesForVisibilityEnconderResult(result) {
    ...
    for (let i = 0; i < result.length - (result.length % 6); ) {
      ...
      for (let j = 0; j < 6; j++) {
        // we have 6 lookAt positions
        // if (i == result.length){
        //     break
        // }
        const lookAtIndex = Math.floor(i / 6) * 3;
        this.lookAtResults[j].positions[lookAtIndex + 0] =
          result[i].camera_coordinates[0];
        this.lookAtResults[j].positions[lookAtIndex + 1] =
          result[i].camera_coordinates[1];
        this.lookAtResults[j].positions[lookAtIndex + 2] =
          result[i].camera_coordinates[2];
    ...
    this.points = new THREE.Points(
      this.lookAtResults[this.currentLookAt].geometry,
      material
    );
