...
import {
  BUILDING_COLOR_TO_OBJECT,
  BUILDING_OBJECT_TO_COLOR,
  COLOR_TO_OBJECT,
  DEPTH_SKY,
  OBJECT_TO_COLOR,
  REAL_WORLD_OBJECT_TO_COLOR,
  VIEW_MODES,
} from "./Utils/constants.js";
import {
  createCsvColor,
  increaseMapFrequency,
  isGreyColor,
  roundColor,
...
export default class Experience {
    ...
    setTimeout(() => {
      ...
      let buildingData = this.buildingMeshes.map((mesh) =>
        Object.assign(
          ...
          mesh.userData,
          {
            "Local Location": mesh.geometry.boundingSphere
              ? mesh.geometry.boundingSphere.center
              : new THREE.Vector3(0, 0, 0),
          },
          {
            "World Location": mesh.geometry.boundingSphere
          ...
          {
            "World Points":
              this.extractWorldCoordinatesFromMeshGeometries(mesh),
          }
        )
      );
  ...
  countColorVisiblityMode(readPixelBuffer, shouldLogResults) {
    ...
    if (shouldLogResults) {
      ...
      for (const color in colorMap) {
        if (color in COLOR_TO_OBJECT) {
          console.log(
            `${COLOR_TO_OBJECT[color]}: ${
              (colorMap[color] * 100) / totalPixels
            }% | ${colorMap[color]} pixels`
          );
        } else {
          console.log(
            `${color}: ${(colorMap[color] * 100) / totalPixels}% | ${
              colorMap[color]
            } pixels`
          );
        }
    ...
    if (shouldLogResults) {
      ...
      for (const color in colorMap) {
        if (color in BUILDING_COLOR_TO_OBJECT) {
          console.log(
            `${BUILDING_COLOR_TO_OBJECT[color]}: ${
              (colorMap[color] * 100) / totalPixels
            }% | ${colorMap[color]} pixels`
          );
        } else {
  ...
  countColorDepthMode(readPixelBuffer) {
    ...
    console.log(
      `Average grey color: ${averageGreyColor}, ${averageGreyColor}, ${averageGreyColor}`
    );
  ...
  enableVisibilityMode() {
    ...
    // this.renderer.updateClearColor(`#ffffff`)
    this.renderer.saoPass.enabled = false;
  ...
  enableBuildingDataMode() {
    ...
    this.world.lights.setDirectionalLight(false);
    this.renderer.updateClearColor(
      `rgb(${BUILDING_OBJECT_TO_COLOR["miscelaneous"]})`
    );
    this.renderer.saoPass.enabled = false;
  ...
  callTestEncoderOnCurrentPosition() {
    ...
    this.visibilityEncoderService
      .testEncoderOnCurrentPosition(requestData)
      .then((result) => {
