...
import { getBrushedPoints, onBrushedPoints } from "../../store";

// Global Variables
let global_locations = []; // all locations
...
export default class HiddenMap {
  ...
  createHiddenMap() {
    // Should receive global data and query data.
    const svg = this.svg;
    ...
    d3.json("test_set_as_query.json").then((data) => {
      ...
      console.log(
        "Populating hidden map with ",
        global_locations.length,
        " locations."
      );
      ...
      xDomain[0] = xDomain[0] - 5;
      xDomain[1] = xDomain[1] + 5;
  ...
  customShape(x, y) {
    ...
    return `M${x},${y - size} L${x - size},${y + size} L${x + size},${
      y + size
    } Z`; //triangle
  ...
  displayGlobalLocations() {
    ...
    if (displayGlobalParticles) {
      this.experience.queryLocationParticles =
        this.particleHelper.plotParticles(selected_locations);
    }
    this.experience.world.updatePovInterfaceAfterBrushOnHistogram(
      selected_locations
    );

    console.log("gets here on brushing");
  }
  ...
  brushEnded(event) {
    ...
    if (false) {
      //(selected_query_locations.length > 5){
      this.experience.queryLocationParticles =
        this.particleHelper.plotParticles(selected_query_locations);
      this.experience.world.updatePovInterfaceAfterBrushOnHistogram(
        selected_query_locations
      );
    } else {
      ...
      //Kazi 3D:
      this.experience.queryLocationParticles =
        this.particleHelper.plotParticles(particlesToPlot);
