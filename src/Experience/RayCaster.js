...
import {
  createArrayOfPointsFromGroup,
  createOBBHelper,
  updateChildrenMaterial,
} from "./Utils/helpers";
...
export default class RayCaster {
  constructor() {
    ...
    this.canvas = document
      .getElementsByClassName("webgl")[0]
    ...
    window.addEventListener("mousedown", (event) => {
      ...
      if (
        ...
        this.hoveredBuilding.name !== this.clickedBuilding?.name
      ) {
        ...
        this.pointsByNormal = this.groupPointsByNormal(
          boundingBoxHelper.geometry,
          boundingBoxHelper
        );
        ...
        updateChildrenMaterial(
          this.clickedBuilding,
          this.materialHelper.materialMap[this.experience.currentMode].click
        );
  ...
  callGetFacadesForClickedBuilding() {
    ...
    this.visibilityEncoderService
      .predictFacadeFromBasePoints(basePoint, this.clickedBuildingHeight)
      .then((res) => {
  ...
  callGetFacadesForClickedBuildingV2AsTiles() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const right = new THREE.Vector3()
        .crossVectors(worldUp, direction)
        .normalize();
    ...
    this.visibilityEncoderService
      .predictFacadeFromBasePointsV2AsTiles(
        basePoint,
        this.clickedBuildingHeight
      )
      .then((res) => {
        // console.log(res.data)
  ...
  groupPointsByNormal(geometry, boxHelper) {
    ...
    for (let i = 0; i < position.array.length; i += 3) {
      ...
      pointsByNormal.set(JSON.stringify(normalCoord), [
        ...pointsByNormal.get(JSON.stringify(normalCoord)),
        positionCoord,
      ]);
    }
  ...
  updateTransformControl() {
    ...
    this.clickedBuilding.position
      .copy(new THREE.Vector3(m1.position.x, m1.position.y, m1.position.z))
      .add(new THREE.Vector3(m2.position.x, m2.position.y, m2.position.z))
      .add(new THREE.Vector3(m3.position.x, m3.position.y, m3.position.z))
      .multiplyScalar(1 / 3);
  ...
  createSlidingBoundingBox(points) {
    ...
    this.heightStep =
      (this.maxHeight - this.helper.position.y) / this.numSegments;
  ...
  screenShotBuilding() {
    ...
    this.screenshotHelper.generateBuildingImages(
      this.buildingCameraPositions,
      this.clickedBuilding.name
    );
  }
  ...
  callGetFacadesForClickedBuildingAsContinousTiles() {
    ...
    this.visibilityEncoderService
      .predictFacadeFromBasePointsAsContinousTiles(
        basePoint,
        this.clickedBuildingHeight
      )
      .then((res) => {
  ...
  removeTilesFromScene() {
    // Function to remove all tiles from the scene:
  ...
  changeFacadeThematic() {
    if (this.guiFacadeControls.facadeTiles == null) {
      ...
      if (this.guiFacadeControls.facadeTilesIntensityColoredBackup != null) {
        this.guiFacadeControls.facadeTiles =
          this.guiFacadeControls.facadeTilesIntensityColoredBackup;
        this.guiFacadeControls.facadeTilesIntensityColoredBackup = null;
      }
      this.guiFacadeControls.addTilesToScene(
        this.guiFacadeControls.facadeTiles
      );
    }
  ...
  changeFacadeThematicFollowingPcpQuery() {
    const buildingQueryCheckbox = document.getElementById(
      "building-query-checkbox"
      ...
      if (this.guiFacadeControls.facadeTiles == null || pcpQuery == null) {
        ...
        for (let tileObjectID in pcpHighlightedFacadeTiles) {
          ...
          for (let semanticId in pcpQuery["f_xyz"]) {
            ...
            let lowerT =
              pcpQuery["f_xyz"][semanticId] -
              pcpQuery["intervals"][semanticId] / 2;
            let upperT =
              pcpQuery["f_xyz"][semanticId] +
              pcpQuery["intervals"][semanticId] / 2;
          ...
          for (let semanticId in pcpHighlightedFacadeTiles[tileObjectID][
            "colors"
          ]) {
            ...
            pcpHighlightedFacadeTiles[tileObjectID]["colors"][semanticId] =
              tileShouldBeHighlighted
                ? highlightedTileColor
                : notHighlightedTileColor;
  ...
  setGUI() {
    ...
    this.guiFacadeControls = {
      ...
      addTilesToScene: function addTilesToScene(
        tilesData,
        analysisMode = false
      ) {
        if (analysisMode) {
        ...
        tilesData.forEach((tile) => {
          ...
          if (analysisMode) {
            //restore facadeTiles data after generating the highlights.
    ...
    this.gui.facadesFolder
      .add(this.guiFacadeControls, "sliderValue", 0, 7, 1) // Min: 0, Max: 100, Step: 1
      .name("Semantic ID");
    // .onChange(value => console.log("New Semantic ID: ", value));
    // Add text input
    this.gui.facadesFolder
      .add(this.guiFacadeControls, "textValue")
      .name("Semantic Name:");

    // Add button to log text
    this.gui.facadesFolder
      .add(this.guiFacadeControls, "displayFacade")
      .name("Display Facade");

    // Add button to log text
    this.gui.facadesFolder
      .add(this.guiFacadeControls, "removeTilesFromScene")
      .name("Remove Tiles");
    ...
    this.gui.sampleControls
      .add(guiControls, "sliderValue", 0, 10, 1) // Min: 0, Max: 100, Step: 1
      .name("Slider");
  ...
  update() {
    ...
    const locationParticles = this.experience.queryLocationParticles
      ? this.experience.queryLocationParticles
      : [];
    const intersects = this.instance.intersectObjects([...this.buildingMeshes]);
    const intersectsParticle = this.instance.intersectObjects([
      ...locationParticles,
    ]);
